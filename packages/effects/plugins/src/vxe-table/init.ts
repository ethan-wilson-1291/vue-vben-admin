...
export function setupVbenVxeTable(setupOptions: SetupVxeTable) {
  ...
  const localMap: Record<string, ReturnType<typeof normalizeVxeLocale>> = {
    'zh-CN': normalizeVxeLocale(zhCN),
    'en-US': normalizeVxeLocale(enUS),
    'es-ES': normalizeVxeLocale(enUS),
    'fr-FR': normalizeVxeLocale(enUS),
    'it-IT': normalizeVxeLocale(enUS),
    'vi-VN': normalizeVxeLocale(enUS),
  };
  ...
  watch(
    ...
    ([isDarkValue, localeValue]) => {
      ...
      const i18n = localMap[localeValue] ?? normalizeVxeLocale(enUS);
      VxeUI.setI18n(localeValue, i18n);
